...
export function InventoryPanel({ inventory, reload }) {
  const [form, setForm] = useState({ category: "wetsuit", suit_size: "XS", min_shoe_size: 5, max_shoe_size: 7, quantity: 1 }); const [error, setError] = useState("");
  ...
  async function changeQuantity(item, quantity) { try { await api(`/api/officer/inventory/${item.id}`, quantity <= 0 ? { method: "DELETE" } : { method: "PUT", body: JSON.stringify({ ...item, quantity }) }); reload(); } catch (caught) { setError(caught.message); } }
